...
export default function GeneralBanking() {
  ...
  const { data } = useSwr(`WebUsers/GetAllWebUsers`);
  const [listData, setListData] = useState();

  useEffect(() => {
    var options = [];
    data?.columnDict?.map((item, i) => {
      options.push({ name: item.field, value: item.enumList, header: item.headerName });
    });
    setListData(options);
  }, [data]);

  return (
    <Page title="Genel:Kullanıclar">
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <Grid Container>
          {data && (
            <Grid item xs={12} md={12}>
              <UserList data={data} listData={listData} />
            </Grid>
          )}
        </Grid>
      </Container>
